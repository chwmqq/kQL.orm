...
namespace kQL.orm.demo
{
    public class SampleCode
    {
        ...
        public static void 测试_插入()
        {
            ...
            var json = dy.Query(dyQuery).AsJson();
            Console.WriteLine(json);
            Thread.Sleep(50);
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                var json2 = dy.Query(dyQuery2).AsJson();
        ...
        public static void 测试_存储过程_视图()
        {
            ...
            Console.WriteLine(dy.Query(query5).AsJson());
        }
        ...
        { 
            ...
            //IN 查询
            var query4 = new DyQuery<tb_user>().Where(t1 => t1.用户名.Dy_Right(1).Dy_In(new List<string> { "1", "2", "3" }))
                .Select(t1 => t1.账号).AsQuery();
            Console.WriteLine(dy.Query(query4).AsJson());
            ...
            Console.WriteLine(dy.Query(query6).AsJson());
            ...
            var query7 = new DyQuery<tb_order>(t2 => t2)
                         ...
                         .AsQuery();
            //.AsQueryPaged(1,10); 
            Console.WriteLine(dy.Query(query7).AsJson()); 
        }
        #region 调用自定义函数
        ...
        public static void 测试_结果集()
        {
            ...
            var plist1 = dy.Query(query5).AsTPageList<tb_user>();
            ...
            var plist2 = dy.Query(query6).AsTPageList<tb_user>();
            ...
            */
            ...
            var keyValues = dy.Query(query7).AsKeyValues();
            ...
            var dynamicObj = dy.Query(query8).AsDyT();
            ...
            var dynamicObjList = dy.Query(query9).AsDyTList();
            ...
            var treeNodeRoot = dy.Query(query10).AsTree<tb_categories>((t1, t2) => t1.父ID == t2.子ID);
            ...
            var query11 = new DyQuery<tb_order>(t2 => t2)
                         ...
                         .AsQuery();
            //AsT_OneOne 单条 
            var r0 = dy.Query(query11).AsT_OneOne<tb_order, tb_order_detail>();
            ...
            //AsTList_OneOne 多条
            var r1 = dy.Query(query11).AsTList_OneOne<tb_order, tb_order_detail>();
            ...
            var r2 = dy.Query(query11).AsT_OneMany<tb_order, tb_order_detail>();
            ...
            var r3 = dy.Query(query11).AsTList_OneMany<tb_order, tb_order_detail>(one => new tb_order { 账号 = one.账号 });
            /** new 主表类型，这里 new tb_order , 只要属性字段，“one.账号”无实际意义  ，也可以写成以下形式，与上一条等价 **/
            /****即，只认对象类型中的属性名称 ****/
            var r4 = dy.Query(query11).AsTList_OneMany<tb_order, tb_order_detail>(one => new tb_order { 账号 = "" });
            foreach (var oneMany in r4) {
                Console.WriteLine("账号:{0},订单数量:{1}", oneMany.Item1.账号, oneMany.Item2.Count);
            }

            //AsTList_OneMany 根据多个字段分组
            var r5 = dy.Query(query11).AsTList_OneMany<tb_order, tb_order_detail>(one => new tb_order { 账号 = one.账号, 订单ID = one.订单ID });
            //以下等价
            var r6 = dy.Query(query11).AsTList_OneMany<tb_order, tb_order_detail>(one => new tb_order { 账号 = "", 订单ID = Guid.Empty });
